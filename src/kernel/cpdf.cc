...
PageTreeNodeType getNodeType(const boost::shared_ptr<IProperty> &nodeProp)throw()
{
	...
	if(nodeDict->containsProperty("Type"))
	{
		try
		{
			boost::shared_ptr<CName> nodeType=nodeDict->getProperty<CName>("Type");
	...
	if(nodeDict->containsProperty("Kids"))
	{
		try
		{
			boost::shared_ptr<CArray> kidsProp=nodeDict->getProperty<CArray>("Kids");
			return InterNode;
		}catch(...)
		{
...
void getKidsFromInterNode(const boost::shared_ptr<CDict> & interNodeDict, Container & container)throw()
{
	...
	if(interNodeDict->containsProperty("Kids"))
	{
		try 
		{
			boost::shared_ptr<CArray> kidsArray = interNodeDict->getProperty<CArray>("Kids");
...
void CPdf::invalidate()
{
	...
	// boost::shared_ptr<CPdf> was dropped) we can't call unregisterPageObservers 
	// which relies on it (like everything which uses boost::weak_ptr to this 
...
using namespace utils;
	...
	{
		// adds new Count property with correct value
		boost::scoped_ptr<IProperty> countInt(CIntFactory::getInstance((int)count));
	...
	for(i=kids.begin(); i!=kids.end(); ++i, ++index)
	{
		...
		{
			// adds new Count property with correct value
			boost::scoped_ptr<IProperty> countInt(CRefFactory::getInstance(interNodeRef));
	...
	if(countChanged && propagate)
	{
		...
		if(interNode->containsProperty("Parent"))
		{
			try
			{
				boost::shared_ptr<CDict> parentDict=interNode->getProperty<CDict>("Parent");
...
using namespace utils;
	...
	boost::shared_ptr<CDict> interNode_ptr=currentPage_ptr->getProperty<CDict>("Parent");
	boost::shared_ptr<CArray> kids_ptr;
	try {
		kids_ptr=interNode_ptr->getProperty<CArray>("Kids");
	}catch(...) {
